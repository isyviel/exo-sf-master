...
Renseigner la clef dans la variable d'environnement `DARK_SKY_API_KEY` (suivre les instructions fournies dans le fichier `.env`)
...
- [ ok ] Formater les données récupérées par le service afin que le tableau en retour renvoi de vraies valeurs
- [ ok ] Passer les données au template via le controller
- [ ok ] Mettre à jour le template pour afficher les données
- [ ok ] Remonter la météo de toulouse en Occitanie
- [ ok ] Gérer les erreurs de l'API via un bloc `try catch` dans le service
- [ ok ] Ajouter un formulaire et des appels API pour permettre à l'utilisateur de choisir sa ville
- [ ok ] Faire de la page qui affiche la météo la home page
- [ ok ] Rendre la page qui affiche la météo responsive
